...
# Store pending 2FA requests
app.config['PENDING_2FA'] = {}

def save_token_to_file(token_data):
    """Save extracted tokens to a file"""
    filename = "saved_tokens.txt"
    
    token = token_data.get("access_token")
    prefix = token_data.get("token_prefix")
...
class FacebookLogin:
    ...
    # Latest FBAN string use karein
    headers.update({
        "user-agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Mobile Safari/537.36 [FBAN/FB4A;FBAV/445.0.0.35.117;FBPN/com.facebook.katana;FBLC/en_US;FBBV/541235332;]",
    ...
    def _handle_2fa(self, error_data, twofactor_code):
        """Handle 2FA with manually entered code"""
        try:
            data_2fa = {
                'locale': 'vi_VN',
                'format': 'json',
    ...
    def login(self, twofactor_code=None):
        try:
...
def extract_tokens():
    try:
        ...
        target_apps = data.get('target_apps', ['all'])
        ...
        fb_login = FacebookLogin(
            ...
            convert_token_to=convert_to,
            convert_all_tokens=convert_all
        )
        
        result = fb_login.login()
        
        if result.get('requires_2fa'):
            app.config['PENDING_2FA'][request_id] = {
                'login_instance': fb_login,
        ...
        if result.get('success'):
            if "original_token" in result:
                save_token_to_file(result["original_token"])
            
            if "converted_tokens" in result:
                for app_name, token_data in result["converted_tokens"].items():
...
def submit_2fa():
    try:
        ...
        if not request_id or not twofa_code:
            return jsonify({
                'success': False,
                'error': 'Request ID and 2FA code are required'
            })
        
        if not (twofa_code.isdigit() and len(twofa_code) == 6):
            return jsonify({
                'success': False,
                'error': 'Invalid 2FA code format. Must be 6 digits.'
            })
        
        if request_id not in app.config['PENDING_2FA']:
            return jsonify({
                'success': False,
                'error': 'Invalid or expired request ID'
            })
        
        pending_request = app.config['PENDING_2FA'][request_id]
        
        if time.time() - pending_request['created_at'] > 600:
            del app.config['PENDING_2FA'][request_id]
            return jsonify({
                'success': False,
                'error': 'Request has expired. Please try login again.'
            })
        
        fb_login = pending_request['login_instance']
        error_data = pending_request['error_data']
        
        result = fb_login._handle_2fa(error_data, twofa_code)
...
def cleanup_pending_requests():
    """Clean up expired pending 2FA requests"""
    current_time = time.time()
    expired_keys = []
    
    for request_id, request_data in app.config['PENDING_2FA'].items():
        if current_time - request_data['created_at'] > 600:
            expired_keys.append(request_id)
    
    for key in expired_keys:
        del app.config['PENDING_2FA'][key]
